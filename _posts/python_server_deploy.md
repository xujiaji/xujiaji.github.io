...
4. 编辑安全组，添加22端口和80端口的访问权限
    - 在EC2管理界面将你创建的实例拉拉到最右边有个叫安全组的栏目，点进去![an_quan_zu1](image\python-server-deploy\an_quan_zu1.png)
    - 进去后点击入站-编辑![an_quan_zu2](image\python-server-deploy\an_quan_zu2.png)
    - 创建SSH入站规则，绑定自己的IP地址，第3步选了后会自动获取你的ip地址。如下图![an_quan_zu3](image\python-server-deploy\an_quan_zu3.png)